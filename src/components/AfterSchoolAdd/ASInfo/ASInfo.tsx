...
import { asAddTypeState, asInitState, studentAddListState } from '../../../recoil/asAddState';
...
const ASInfo = () => {
  ...
  const ActivityDay = () => {
    return (
      <BaseBox handleSelect={handleActivityDay} option={asInitData.day}>
        <option value='default' selected hidden>요일</option>
  ...
  const onASCreate = (type: string, data: any) => {
    if(type === 'major'){
      if(data.name || data.location_id || data.teacher_id) {return alert('빈칸을 다 채워주세요.')}
      setLoading(true)
      majorApi.postMajor(data.name, data.head_name, data.head_gcn, data.location_id, data.teacher_id)
      .then((res) => {
        studentAddList.map((i: any, index: number) => {
          majorApi.postMajorMember(res.data.major_id, i.student_id)
          .then(() => {
      ...
      if(data.name || data.teacher_id || data.location_id || data.day) { return alert('빈칸을 다 채워주세요') }
      setLoading(true)
      activityApi.postActivity(data.name, data.teacher_id, data.location_id, data.day)
      .then((res) => {
        studentAddList.map((i: any, index: number) => {
          activityApi.postActivityStudent(res.data.after_school_id, i.studnet_id)
          .then(()=>{
            if(index === studentAddList.length-1 ) {
  ...
  return (
    <>
        <S.Wrapper>
          ...
          <S.Right>
            <ASTypeSelect/>
          <S.ASAddButton onClick={() => onASCreate(type, asInitData)}>생성하기</S.ASAddButton>
          </S.Right>
        </S.Wrapper>
        {loading && <Loading/>}
    </>
